...
Install the synthesis extras first:

```bash
pip install -e ".[synthesis]"
```

The pipeline extends a trained BTF to a tileable Wang tiling in five stages:
extract latents → train a diffusion model → build Wang tiles → repaint seams → render.
Run all stages from the repository root:

```bash
EXPERIMENT=<savedir>/<prefix>/<run>
CHECKPOINT="$EXPERIMENT/result/<prefix>_iter_<n_steps>.pth"
PROJECT="$EXPERIMENT/synthesis/project.json"

python synthesis/extract_latents.py --checkpoint "$CHECKPOINT" --config synthesis/configs/default.json
...
Extraction infers `<experiment>/synthesis` from the checkpoint path and writes `project.json`.
All later stages read from that file.
See [`synthesis/README.md`](synthesis/README.md) for configuration reference and stage-by-stage details.
